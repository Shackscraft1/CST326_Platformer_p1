    ...
    void LoadLevel()
    {
        ...
        while (levelRows.Count > 0)
        {
            ...
            for (var columnIndex = 0; columnIndex < rowChars.Length; columnIndex++)
            {
                ...
                switch (currentChar)
                {
                    //remember to instantiate the prefabs as parents
                    case('x'):
                        Vector3 newPositionR = new Vector3(columnIndex + 0.5f, row + 0.5f, 0);
                        Transform rockInstance = Instantiate(rockPrefab, levelRoot).transform;
                        rockInstance.position = newPositionR;
                        break;
                    case('?'):
                        Vector3 newPositionG = new Vector3(columnIndex + 0.5f, row + 0.5f, 0);
                        Transform goldInstance = Instantiate(questionBoxPrefab, levelRoot).transform;
                        goldInstance.position = newPositionG;
                        break;
                    case('s'):
                        Vector3 newPositionS = new Vector3(columnIndex + 0.5f, row + 0.5f, 0);
                        Transform stoneInstance = Instantiate(stonePrefab, levelRoot).transform;
                        stoneInstance.position = newPositionS;
                        break;
                    case('b'):
                        Vector3 newPositionB = new Vector3(columnIndex + 0.5f, row + 0.5f, 0);
                        Transform brickInstance = Instantiate(brickPrefab, levelRoot).transform;
                        brickInstance.position = newPositionB;
                        break;
                }
